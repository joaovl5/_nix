...
This is not AI-specific. Future always-on agents such as a `hermes`-style
service should use the same container model as ordinary server services.
...
- Put composition above units: a container decides what units run together and
  how the host integrates them.
- Optimize the helper API for the common case: one container containing one
  unit.
- Still define the normalized shape for multi-unit containers.
- Keep ingress, persistence, backups, secret projection, and shared providers
  host-owned.
...
- Do not require perfect firewall enforcement on the first implementation,
  even though communication schemas are specified here.
...
`target` is the same concept currently used by `my.vhosts`: a vhost target
label such as `actual`, not a TLD/FQDN.

If `target` is omitted, the helper creates the container without host HTTP
exposure.

## Explicit multi-unit shape

The helper optimizes for one unit, but the normalized container shape supports
multiple units.
...
- `expose` is a list to avoid repeating the same name as both an attr key and
  a field.
- In the one-unit helper, `target = "..."` is sugar for exposing that unit's
...
`default.nix` may reuse `unit.nix`, but it remains allowed to emit host
integration such as `my.vhosts`, host-local backups, and other current host
assumptions.
...
TCP and UDP endpoints use the same endpoint idea, but they do not lower
through `my.vhosts`.
...
- reserve `10.88.0.0/16` for this container network unless rollout finds a
  conflict;
...
- `host.containers` resolves inside the guest to that container's host-side
  address;
...
A container state declaration lowers to backup config for the host path, not
the guest path.
...
`privateUsers = "pick"` is safer, but writable host bind mounts are simpler
...
A full container compromise can read any secret exposed to that container. The
env-file model is about operational simplicity and avoiding guest-side SOPS
key management, not about making exposed secrets unreadable after compromise.

## Host providers

Host providers are specified as part of the v1 schema, even if implementation
arrives in stages.
...
- the host provider decides how to create users, grants, passwords, and
  `pg_hba` entries.
...
Cross-container communication is specified as a schema now, but firewall
enforcement may arrive later.
...
- use `container = "..."` only when the local dependency name differs from the
...
The spec defines the data contract. The first implementation may validate some
communication declarations before enforcing all of them.
...
The container layer only sees units, endpoints, state, secrets, exposures, and
provider edges.
