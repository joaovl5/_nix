...
- The first blocker is the user unit `docker.service` during the NixOS
  `reloading user units for temperance` phase, before any later host failures:
  activation stops the old user unit, tries to start the new one, then reports
  `Failed to start user unit docker.service`.
- Deploy rolls back from generation 35 to 34; the rolled-back/current
  `docker.service` is active again, so current live success is rollback state,
  not evidence that the forward generation would activate.

### Evidence

- Remote user journal for the failed starts at `07:58:39/41/43` and
  `08:03:55/57/59` shows:
  `/nix/store/g3dg45f1cnzhdj4lljs8yz41l147li5g-dockerd-rootless-dockerd:
  line 9: /etc/resolv.conf: Permission denied`.
- That wrapper is generated by `modules/aspects/systems/host-base.nix` and is
  injected with `systemd.user.services.docker.environment.DOCKERD`.
- Evaluated new generation:
  - `virtualisation.docker.rootless.package.version = 29.5.3`
  - `DOCKERD =
    /nix/store/g3dg45f1cnzhdj4lljs8yz41l147li5g-dockerd-rootless-dockerd`
  - wrapper line 9 writes the generated resolver config to `/etc/resolv.conf`
    before execing `dockerd`.
...
- This is a forward-generation activation blocker specific to the rootless
  Docker user unit on `temperance`.
- The custom resolver wrapper assumed `/etc/resolv.conf` is writable inside
  the rootless Docker namespace when `/etc` is a tmpfs. With Docker 29.5.3,
  `dockerd-rootless.sh` defaults `DOCKERD_ROOTLESS_ROOTLESSKIT_DETACH_NETNS`
  to `true`; under that startup mode, the wrapper reaches the
  `/etc/resolv.conf` write but gets `Permission denied`.
- Extra nuance: the rolled-back Docker `29.4.3` wrapper is structurally
  identical to the new one: same tmpfs guard, same line-9 resolver rewrite,
  and equivalent daemon DNS config. The observed regression is therefore tied
  to the new generation's runtime/package behavior, not a source-level change
  in the wrapper logic itself.
- A follow-up repo sweep found no matches under `home/` or
  `modules/aspects/users/` for `systemd.user.services.docker`, `DOCKERD`,
  `virtualisation.docker`, or `dockerd-rootless`. The Docker user-unit
  customization is confined to the system-side `host-base.nix` override chain,
  keeping the diagnosis scoped to that override plus the new rootless Docker
  runtime behavior.
- Because the unit has `Restart=always`, systemd retries, but deploy
  activation observes the failed/auto-restart state before recovery and
  aborts, matching previous deployment-debugging lessons about retry-only
  mitigations.

### Fix used

- `modules/aspects/systems/host-base.nix` now sets
  `DOCKERD_ROOTLESS_ROOTLESSKIT_DETACH_NETNS=false` in the rootless Docker
  user service environment alongside the custom `DOCKERD` wrapper.
- This keeps rootlesskit's network namespace attached so the wrapper can still
  rewrite the namespace-local `/etc/resolv.conf` before execing dockerd.

### Verification after fix

- Local targeted eval confirmed `temperance`'s `docker.service` environment
  includes `DOCKERD_ROOTLESS_ROOTLESSKIT_DETACH_NETNS = "false"`.
- `just fmt` passed.
- Targeted `deploy --skip-checks .#temperance --log-dir logs --debug-logs
  --confirm-timeout 1800 --activation-timeout 1800` completed and confirmed
  activation.
- Post-targeted-deploy check on `temperance` showed `docker.service` as
  `active/running`, `Result=success`, `ExecMainStatus=0`, `NRestarts=0`, and
  Docker server version `29.5.3`.
- Full `deploy --skip-checks --log-dir logs --debug-logs --confirm-timeout
  1800 --activation-timeout 1800` completed and confirmed activation for
  `temperance`, `lavpc`, and `tyrant`.
- Post-full-deploy check on `temperance` showed `docker.service` still
  `active/running` with `Result=success`, `ExecMainStatus=0`, `NRestarts=0`,
  and zero failed system units.
- Per request, no `nix flake check` and no deploy without `--skip-checks` were
