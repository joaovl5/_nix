{
  ...
  backup_lib = import ../units/_backup {
    inherit (pkgs) lib;
    config = tyrant;
    inherit pkgs;
  };
  inherit (backup_lib) collect;
  inherit (backup_lib) render_local;
...
in {
  ...
  assert builtins.attrNames lavpc.my."unit.backup".rendered.services == [];
