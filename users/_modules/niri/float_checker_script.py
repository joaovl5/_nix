...
# Credits: https://github.com/niri-wm/niri/discussions/1599
"""
...
RULES = [
    Rule(
        match=[
            Match(
                # # match any titles not starting with "Extension:"
                # title="^(?!.*Extension:).*$",
                app_id="librewolf",
            ),
        ],
...
def update_matched(win):
    is_lw = win["app_id"] == "librewolf"
    is_ext = win["title"].startswith("Extension:")
    id = win["id"]

    if is_lw:
        if is_ext:
            if id not in floated:
                float(id)
                floated.add(id)
        else:
            if id not in tiled:
                tile(id)
                tiled.add(id)
        print(f"LW: title={win['title']}")

    if (
        is_lw and (not is_ext)
        # and not matched_before
    ):
        print(f"tiling title={win['title']}, app_id={win['app_id']}")
        tile(win["id"])
    else:
        print(
            f"failed title={win['title']}, app_id={win['app_id']} | is_lw: {is_lw}, is_ext: {is_ext}",
        )
