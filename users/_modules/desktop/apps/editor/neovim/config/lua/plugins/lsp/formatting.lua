...
local formatters = {alejandra = {command = "alejandra"}, kdlfmt = {command = "kdlfmt", args = {"format", "--kdl-version", "v1", "--stdin"}}, nix_fmt = {command = "nix", args = {"fmt"}}, prettierd = {command = "prettierd"}, sane_fnlfmt = {command = "fnlfmt", args = {"-"}}}
local default_formatters_by_ft = {["*"] = {"keep-sorted"}, _ = {"trim_whitespace", "trim_newlines", "squeeze_blanks"}, dockerfile = {"dockerfmt"}, fennel = {"sane_fnlfmt"}, fish = {"fish_indent"}, handlebars = {"prettierd"}, javascript = {"prettierd"}, json = {"jsonfmt"}, kdl = {"kdlfmt"}, kulala = {"kulala-fmt"}, lua = {"stylua"}, markdown = {"rumdl"}, nix = {"alejandra"}, python = {"ruff_fix", "ruff_format", "ruff_organize_imports"}, rust = {"rust_fmt"}, sh = {"shfmt"}, sql = {"sqruff"}, toml = {"taplo"}, typescript = {"prettierd"}, typescriptreact = {"prettierd"}, yaml = {"yamlfmt"}}
