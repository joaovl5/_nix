-- [nfnl] fnl/plugins/remote.fnl
local _1_ = require("lib.plugins")
local _2_ = require("lib.keys")
local spec_24_auto = {}
local function _3_()
  do
    local name_1_auto = require("remote-sshfs")
    local fun_2_auto = name_1_auto.setup
    fun_2_auto({})
  end
  local name_1_auto = require("telescope")
  local fun_2_auto = name_1_auto.load_extension
  return fun_2_auto("remote-sshfs")
end
for __25_auto, attrs_26_auto in ipairs({_1_.deps({"nvim-telescope/telescope.nvim", "nvim-lua/plenary.nvim"}), _1_.event("VeryLazy"), _1_.config(_3_)}) do
  for key_27_auto, value_28_auto in pairs(attrs_26_auto) do
    spec_24_auto[key_27_auto] = value_28_auto
  end
end
spec_24_auto[1] = "nosduco/remote-sshfs.nvim"
return spec_24_auto
