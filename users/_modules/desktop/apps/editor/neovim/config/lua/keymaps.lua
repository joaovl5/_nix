...
local function treewalker(subcommand)
  ...
  if not ok then
    ...
    then
      return vim.notify(
        "Treewalker: no Treesitter node under cursor",
        vim.log.levels.WARN
      )
    else
      return error(err)
...
wk.add({
  km("<leader>E", { desc = "Explore root" }, _47_),
  km("<leader>e", { desc = "Explore at file" }, _48_),
})
...
local function minifiles_filter(f)
  return (
    show_dotfiles
    or vim.endswith(f.name, ".env")
    or not vim.startswith(f.name, ".")
  )
end
...
local function _50_(args)
  ...
  return wk.add({
    ...
    km(
      "<leader>bS",
      { desc = "Make focused dir CWD", buffer = buf_id },
      set_cwd
    ),
    km(
      "<leader>bO",
      { desc = "Open w/ system handler", buffer = buf_id },
      open_sys
    ),
  })
end
return n.autocmd(
  "User",
  { pattern = "MiniFilesBufferCreate", callback = _50_ }
)
