...
def test_wait_for_profile_bootstrap_times_out_when_token_never_matches(
  ...
  retry_probe_attempts = [
    command for command in wait_attempts if "-e" in command
  ]
...
def test_read_bootstrap_failure_detail_checks_current_then_persisted_then_logs(
  ...
  monkeypatch.setattr(
    docker_runtime, "_read_current_bootstrap_failure", fake_current
  )
  monkeypatch.setattr(
...
@pytest.mark.parametrize(
  ...
  [
    ...
    pytest.param(
      lambda labels: labels.__setitem__(
        profiles.LABEL_PROFILE, "Other Profile"
      ),
      None,
      id="profile-name",
    ),
...
def test_cli_enter_loads_image_starts_container_waits_then_execs(
  ...
  monkeypatch.setattr(
    cli.docker_runtime,
    "start_profile_container",
    lambda *, profile, workspace_root, runtime_spec, bootstrap_token: (
      calls.append(
...
def test_stop_profile_container_surfaces_docker_stop_failures(
  ...
  with pytest.raises(
    docker_runtime.DockerRuntimeError, match="permission denied"
  ):
    docker_runtime.stop_profile_container(DEMO_PROFILE)
