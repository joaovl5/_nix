...
def _missing_required_shared_assets(
  shared_assets_root: Path,
) -> tuple[str, ...]:
  return tuple(
    relative_source
...
def _resolve_shared_mounts(
  ...
  *,
  ...
  resolved_host_home = (
    (host_home or Path.home()).expanduser().resolve(strict=False)
  )
  ...
  *,
  ...
  payload = [
    ...
    [
      ...
      for destination, source in host_override_sources
    ],
  ]
...
def _normalize_catalog_image_key(
  *, images: dict[str, object], image_key: str
) -> str:
...
def resolve_installed_package_assets(
  ...
  search_start = (
    search_anchor if search_anchor.is_dir() else search_anchor.parent
  )
  ...
  for package_root in (search_start, *search_start.parents):
    ...
    missing_required_assets = _missing_required_shared_assets(
      shared_assets_root
    )
