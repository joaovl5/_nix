...
def test_build_vm_runner_uses_expected_installable(tmp_path: Path) -> None:
  ...
  runner = build_vm_runner(
    repo_root=tmp_path, host="lavpc", run_command=fake_run
  )
...
def test_build_vm_runner_uses_vm_script_from_directory_output(
  tmp_path: Path,
) -> None:
...
def test_build_vm_runner_rejects_unexpected_vm_output_shape(
  tmp_path: Path,
) -> None:
  vm_output = tmp_path / "nixos-vm-output"
  vm_output_bin = vm_output / "bin"
  ...
  with pytest.raises(
    UserFacingError, match="Failed to resolve VM runner path"
  ):
...
def test_list_hosts_wraps_command_failures(tmp_path: Path) -> None:
  ...
  with pytest.raises(
    UserFacingError, match="Failed to list VM hosts. nix failed"
  ):
