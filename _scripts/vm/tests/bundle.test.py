...
def write_secret(
  path: Path, content: str = "secret", mode: int = 0o600
) -> Path:
...
def test_resolve_inputs_uses_home_age_default(tmp_path: Path) -> None:
  ...
  resolved = resolve_inputs(
    host="lavpc", home_dir=home, age_key=None, ssh_key=None
  )
...
def test_validate_secret_file_rejects_non_regular_file(
  tmp_path: Path,
) -> None:
...
def test_validate_secret_file_rejects_group_readable_file(
  tmp_path: Path,
) -> None:
...
def test_stage_bundle_copies_expected_layout(tmp_path: Path) -> None:
  age_key = write_secret(
    tmp_path / "inputs" / "age.txt", content="age-secret"
  )
  ssh_key = write_secret(
    tmp_path / "inputs" / "id_ed25519", content="ssh-secret"
  )
...
def test_stage_bundle_replaces_preexisting_destination_symlink(
  tmp_path: Path,
) -> None:
  age_key = write_secret(
    tmp_path / "inputs" / "age.txt", content="age-secret"
