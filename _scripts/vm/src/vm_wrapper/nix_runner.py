...
RunCommand = Callable[
  [Sequence[str], Mapping[str, str] | None, Path | None, bool],
...
def _with_qemu_overrides(
  ...
  combined_opts = " ".join(
    opt for opt in [qemu_opts, override_opts] if opt
  ).strip()
  if combined_opts:
...
def _default_run_vm_command(
  runner_path: Path, env: Mapping[str, str] | None
) -> int:
  completed = run(
    [str(runner_path)],
    check=False,
...
def require_host(host: str, available_hosts: Sequence[str]) -> None:
  ...
  available = (
    ", ".join(sorted(available_hosts)) if available_hosts else "none"
  )
  raise UserFacingError(
  ...
  try:
    ...
    raise UserFacingError(
      _format_command_error(
        f"Failed to build VM runner for host '{host}'", exc
      )
    ) from exc
