...
class CloneRepositories:
  ...
  def should_skip(self, context: InstallerContext) -> bool:
    ...
    secrets_skip = (
      context.secrets is None or not context.secrets.needs_clone()
    )
  ...
  def execute(self, context: InstallerContext, cli: CLI) -> None:
    ...
    cli.run_command(
      command=ShellCommand(
        ...
        env={
          "SOPS_AGE_KEY_FILE": f"{Path.home()}/.age/{context.age_keyfile}"
        },
      ),
...
class ConfigureSubstituters:
  ...
  def execute(self, context: InstallerContext, cli: CLI) -> None:
    ...
    config_base = (
      f"{flake_dir}#nixosConfigurations.{context.flake_host}.config"
    )

    substituters = _nix_eval_tostring(
      f"{config_base}.nix.settings.substituters"
    )
