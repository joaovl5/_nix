...
Entities declare what exists. Den has built-in host, user, and home
entities.
...
Each entity has an aspect, usually `den.aspects.<name>` by default,
and resolves through Den's pipeline.

## Schema

`den.schema.<kind>` defines typed metadata and defaults for all
entities of a kind.
...
Use it only after the schema is stable; it is too rigid during
...
Only move values whose source of truth is the entity, not the module
fixpoint.
...
4. Remove the old option after searches and targeted evals prove it is
   unused
...
Use this for entity-kind pointcuts. Use `den.default.includes` only
when the concern truly applies across entity kinds.
