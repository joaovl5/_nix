...
in {
  ...
  buildPhase = ''
    ...
    pnpm --filter @kaneo/permissions build
  ...
  in
    ...
    ''
      ...
      mkdir -p \
        ...
        "${runtime_root}/packages/permissions" \
      ...
      cp -a packages/permissions/package.json "${runtime_root}/packages/permissions/"
      cp -a packages/permissions/node_modules "${runtime_root}/packages/permissions/"
      cp -a packages/permissions/dist "${runtime_root}/packages/permissions/"
      ...
      #!${stdenv.shell}
      set -euo pipefail
      exec ${nodejs}/bin/node --enable-source-maps "${runtime_root}/apps/api/dist/index.js" "\$@"
      EOF
  ...
  installCheckPhase = ''
    ...
    ${nodejs}/bin/node -e 'Promise.all([import("bcrypt"), import("pg"), import("@kaneo/email")]).then(() => process.stdout.write("runtime imports ok\n"))'
