{
  ...
  den.aspects.system-host-base = {
    ...
    nixos = {
      ...
      docker_rootless_resolv_conf = pkgs.writeText "docker-rootless-resolv.conf" (
        lib.concatMapStringsSep "\n" (server: "nameserver ${server}") docker_dns_servers + "\n"
      );
      docker_rootless_dockerd = pkgs.writeShellScript "dockerd-rootless-dockerd" ''
        set -euo pipefail

        if [ "$(${pkgs.coreutils}/bin/stat -c %T -f /etc)" != "tmpfs" ]; then
          echo "refusing to rewrite /etc/resolv.conf outside rootless Docker namespace" >&2
          exit 1
        fi

        ${pkgs.coreutils}/bin/cat ${docker_rootless_resolv_conf} > /etc/resolv.conf

        exec ${config.virtualisation.docker.rootless.package}/bin/dockerd "$@"
      '';
            ...
            o.merge [
                ...
                virtualisation = o.def {
                  ...
                  docker = {
                    ...
                    rootless = {
                      ...
                      daemon.settings.dns = docker_dns_servers;
                    };
                    autoPrune = {
                      enable = true;
                      dates = "weekly";
              ...
              (o.when opts.home-manager.enable {
                home-manager.users.${cfg.username} = _: {
                  ...
                  imports = opts.home-manager.extra_modules;
                };
              })
            ]
        ))
